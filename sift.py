...
def wdata(kp1,d1,inliers,acc,t):
	...
	if t:
		...
		wb = Workbook()
		wb = load_workbook('sift_output.xlsx')

		ws = wb.create_sheet(title=str(img2Path))
		ws.append(["Test Image","X2","Y2","Scale2","Orientation2","Inliers","Score"])		
		ws.cell(row=kpid, column=1, value=img2Path) #image_id

		for k2 in kp2:
			
			ws.cell(row=kpid, column=2, value=k2.pt[0]) #x2
			ws.cell(row=kpid, column=3, value=k2.pt[1]) #y2			
			ws.cell(row=kpid, column=4, value=k2.size) #scale2
			ws.cell(row=kpid, column=5, value=k2.angle) #orientation2
			kpid = kpid + 1


		ws.cell(row=2, column=6, value=inliers) #score
		ws.cell(row=2, column=7, value=acc) #score
		wb.save(filename = 'sift_output.xlsx')
		#pd.read_excel('sift_output.xlsx', sheetname='Data Collection').to_csv('sift_output.csv', index=False)

	else:
		ws.title = "Query Image"
		ws.append(["Query Image","X1","Y1","Scale1","Orientation1"])

		ws.cell(row=kpid, column=1, value=img1Path) #house_id

		for k1 in kp1:
			
			ws.cell(row=kpid, column=2, value=k1.pt[0]) #x1
			ws.cell(row=kpid, column=3, value=k1.pt[1]) #y1			
			ws.cell(row=kpid, column=4, value=k1.size) #scale1
			ws.cell(row=kpid, column=5, value=k1.angle) #orientation1
			kpid = kpid + 1			
		wb.save(filename = 'sift_output.xlsx')
...
if __name__ == '__main__':
	...
	## Prepare Dataset ##
	dataset = []
	listImages = (subprocess.check_output(["ls dataset/", "-x"], shell=True))
	...
	for img2Path in dataset:
		...
		if results.v:
			...
			#cv2.imshow('Query Image + Keypoints',img1kp)
		...
		#Output CSV
		if results.o:
			wdata(kp1,d1,inliers,percent,1)
